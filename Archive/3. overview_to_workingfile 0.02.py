...
# always include Projectleider and Actiepunten Projectleider
for col in ["Projectleider", "Actiepunten Projectleider"]:
...
template_cols = {}
for cell in ws[2]:
    if cell.value in header_map.values():
        template_cols[cell.value] = cell.column
...
for idx, row in df.iterrows():
    ...
    for std, werk in header_map.items():
        if std in df.columns and werk in template_cols:
            ...
            # wrap text for actiepunten
            if werk == "Actiepunten Projectleider" and isinstance(val, str):
...
for werk, col_idx in template_cols.items():
    ...
    if werk == "Actiepunten Projectleider":
        max_len = len(str(ws[f"{col_letter}2"].value or ''))
        for r in range(start_row, total_rows+1):
            for line in str(ws[f"{col_letter}{r}"].value or '').split("\n"):
...
out_fn = f"Werkbestand_AlleProjecten_{today_txt}_v0.06.xlsx"
