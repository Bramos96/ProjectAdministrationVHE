...
# → Pak zowel input als output kolommen
...
# Always include these columns explicitly
for col in ["Projectleider", 
...
for idx, row in df.iterrows():
    ...
    for std, werk in header_map.items():
        if std in df.columns and werk in template_cols:
            ...
            # voor Verwacht resultaat → afronden en als int opslaan
            if werk == "Verwacht resultaat" and pd.notna(val):
                try:
            ...
            # wrap text for tekstvelden
            if werk in ["Actiepunten Projectleider", "Actiepunten Bram", "Actiepunten Overig", "Algemene informatie"] and isinstance(val, str):
            ...
            # euro-opmaak alleen voor Verwacht resultaat
...
for werk, col_idx in template_cols.items():
    ...
    if werk in ["Actiepunten Projectleider", "Actiepunten Bram", "Actiepunten Overig", "Algemene informatie"]:
