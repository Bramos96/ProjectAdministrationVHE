...
for fname in latest:
    ...
    # Alleen datum in Leverdatum
    if "Leverdatum" in sub.columns:
        sub["Leverdatum"] = pd.to_datetime(sub["Leverdatum"], errors='coerce').dt.strftime('%Y-%m-%d')

    if "Selcode" in sub.columns:
        print(" Selcode gevonden in sub dataframe!")
        print(sub["Selcode"].dropna().head())
    else:
        print(" Selcode ontbreekt in sub dataframe.")
...
# 6B) Handmatige kolommen beschermen tegen Sumatra
HANDMATE_COLUMNS = [
    "Algemene informatie",
    "Actiepunten Overig",
    "Verwacht resultaat"
...
# Sumatra-data mag deze kolommen niet invullen → zet ze op NaN
for col in HANDMATE_COLUMNS:
    if col in df_input.columns:
        df_input[col] = pd.NA

# Voor nieuwe projecten handmatige kolommen vullen met lege strings
for col in HANDMATE_COLUMNS:
...
# 11) Verwacht resultaat berekenen (alleen waar hij nog leeg is en niet handmatig aangepast)
if 'Budget Opbrengsten' in df_merged.columns and 'Budget Kosten' in df_merged.columns:
    ...
    if 'Verwacht resultaat handmatig aangepast' not in df_merged.columns:
        df_merged['Verwacht resultaat handmatig aangepast'] = "Nee"

    mask = (
        (df_merged['Verwacht resultaat'].isna() | (df_merged['Verwacht resultaat'] == "")) &
        (df_merged['Verwacht resultaat handmatig aangepast'] != "Ja")
    )
...
# 13) Versielog invullen
if "Versielog" in df_merged.columns:
    today = pd.Timestamp.today().strftime("%Y-%m-%d")
    df_merged["Versielog"] = today
