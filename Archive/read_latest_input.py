...
# --- BEGIN: VEILIGE SYNC OP PROJECTNUMMER (index al gezet) ---
KEY = "Projectnummer"

# Zorg dat de indexnamen correct zijn (beide al index op KEY volgens jouw code)
df_central.index.name = KEY
df_input.index.name = KEY

# Kolommen die wél automatisch gesynct mogen worden
auto_cols = [c for c in df_input.columns if c not in HANDMATE_COLUMNS + ["Whitelist", FLAG_COL]]

# Leg missende kolommen eerst aan in df_central (voorkomt KeyErrors)
for c in auto_cols:
    if c not in df_central.columns:
        df_central[c] = pd.NA

# Beperk tot kolommen die in beide aanwezig zijn
...
if present:
    ...
    # Beide DataFrames zijn al uitgelijnd op KEY (index): direct updaten
